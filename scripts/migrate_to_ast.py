...
if os.path.exists(backend_path):
    ...
    # Running from within backend directory
        ...
        for old_doc in old_documents:
            ...
            if existing_doc:
                ...
                """), {
                    ...
                    "raw_markdown": raw_markdown,
                    "metadata": json.dumps(content_ast.get('metadata', {})),
                    "updated_at": updated_at
                })
            else:
                # Insert new document
                db.execute(text("""
                    INSERT INTO documents (id, title, content_ast, raw_markdown, metadata, created_at, updated_at)
                    VALUES (:id, :title, :content_ast, :raw_markdown, :metadata, :created_at, :updated_at)
                """), {
                    "id": doc_id,
                    "title": title,
                    "content_ast": json.dumps(content_ast),
...
"""
    ...
    db.execute(text("""
        INSERT INTO documents (id, title, content_ast, raw_markdown, metadata, created_at, updated_at)
        VALUES (:id, :title, :content_ast, :raw_markdown, :metadata, NOW(), NOW())
